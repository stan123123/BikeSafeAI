...
public class ProcessingResultPackager : MonoBehaviour
{
    ...
    /// <summary>
    /// Basically we want to have some error prevention for naming incase the user isnt very smart, honestly though this should basically be impossible to trigger AFAIK
    /// </summary>
    ...
    private void CheckIfProcessingFinished(int currentProgress, int totalToProcess)
    {
        ...
        {
            CopyVideoIntoDirectory();
        }
    }

    private void CopyVideoIntoDirectory()
    {
        string videoPath = ProcessingUserSelectionManager.SelectedVideoPath;

        if (!string.IsNullOrEmpty(videoPath) && File.Exists(videoPath))
        {
            string videoDest = Path.Combine(packageRoot, Path.GetFileName(videoPath));
            File.Copy(videoPath, videoDest, true);
        }
        else
        {
            Debug.LogWarning("No valid selected video path found. Video not packaged.");
        }
    }
    ...
    private void createProcessedVideoData(bool isDoneProcessing, int totalFrames)
    {
        ...
        ProcessedVideoData data = new ProcessedVideoData
        {
            ...
            videoDurationSeconds = ProcessingUserSelectionManager.SelectedDeltaSeconds * totalFrames, // TODO: fix this later, we need to know accurately how long the video is but this will do for now
